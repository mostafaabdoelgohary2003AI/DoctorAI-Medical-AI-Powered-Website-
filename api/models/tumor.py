...
# Import schemas
from schemas.request_models import TumorRequest, PredictionResponse
...
# Configure logging
logger = logging.getLogger("tumor_model")
...
@router.post("/brain-tumor", response_model=PredictionResponse)
async def predict_brain_tumor(request: TumorRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        # Preprocess image
        preprocessed_image = image_processor.preprocess_for_tumor(request.image_data)
        ...
        # Log prediction
        logger.info(f"Brain tumor prediction: {predicted_class} with confidence {confidence:.4f}")
        ...
        return {
            ...
            "model_version": settings.MODEL_VERSIONS["tumor"],
...
@router.get("/brain-tumor/metadata")
async def get_model_metadata() -> Dict[str, Any]:
    """Get metadata about the brain tumor model"""
    return {
        "model_name": "Brain Tumor Detection Model",
        "model_version": settings.MODEL_VERSIONS["tumor"],
        "model_file": "Tumor.h5",
