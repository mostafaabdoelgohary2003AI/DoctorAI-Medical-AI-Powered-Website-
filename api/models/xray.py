...
# Import schemas
from schemas.request_models import XRayRequest, PredictionResponse
...
# Configure logging
logger = logging.getLogger("xray_model")
...
async def predict_xray(request: XRayRequest) -> Dict[str, Any]:
    """Endpoint for X-ray analysis prediction"""
    try:
        # Start timing
        start_time = time.time()
        
        # Preprocess image
        preprocessed_image = image_processor.preprocess_for_xray(request.image_data)
        ...
        # Log prediction
        logger.info(f"X-ray prediction: {predicted_class} with confidence {confidence:.4f}")
        ...
        return {
            ...
            "model_version": settings.MODEL_VERSIONS["xray"],
...
@router.get("/xray/metadata")
async def get_model_metadata() -> Dict[str, Any]:
    """Get metadata about the X-ray model"""
    return {
        "model_name": "X-Ray Analysis Model",
        "model_version": settings.MODEL_VERSIONS["xray"],
        "model_file": "X-ray.h5",
